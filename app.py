...
# 1. 채점 기준 및 키워드 사전 정의
# ==========================================
# 유의어 사전 (정규식 패턴) - 조건에서 허용한 의미 인정
synonyms = {
    "easy_task": r"(쉬운|단순한|친숙한|노력이 덜 드는|간단한)",
    "hard_task": r"(어려운|도전적인|복잡한|어려움이 있는)",
    "alone": r"(혼자|타인 없이|독학|스스로|차분하게|조용한)",
    "together": r"(함께|여럿이|타인이 있는|친구들과)",
    "facilitation": r"(사회적 촉진|능률이 오르|효율이 높아|잘 되|유리)",
    "inhibition": r"(사회적 억제|능률이 떨어|방해가 되|효율이 낮아|불리)"
}

# 설명 방법별 필수 구조 키워드 사전 - 방법의 특성이 드러나야 함
methods_structure = {
    "비교와 대조": r"(반면|한편|차이|달리|마찬가지로|공통점)",
    "인과": r"(기 때문에|따라서|그러므로|원인|결과로|해서)",
    "정의": r"(은 이다|는 이다|란|의미는|뜻한다)",
    "예시": r"(예를 들어|예컨대|와 같은|등이 있다)",
    "분석": r"(로 구성|요소로|이루어져)",
    "분류와 구분": r"(기준으로 나뉜다|종류가 있다|구분된다)"
}
...
def grade_q1(ans_a, ans_b, ans_c):
    ...
    # ㉠ 채점: 쉬운 과제 의미
    if re.search(synonyms["hard_task"], ans_a):
        feedback.append("❌ ㉠ 오답: 어려운 과제로 오개념을 작성했습니다.")
    elif re.search(synonyms["easy_task"], ans_a):
        score += 1
    ...
    # ㉡ 채점: 혼자 집중하는 의미
    if re.search(synonyms["together"], ans_b):
        feedback.append("❌ ㉡ 오답: '함께'라는 오개념이 포함되었습니다.")
    elif re.search(synonyms["alone"], ans_b):
        score += 1
    ...
    if re.search(synonyms["facilitation"], ans_c):
        feedback.append("❌ ㉢ 오답: '사회적 촉진'과 혼동하였습니다.")
    elif re.search(synonyms["inhibition"], ans_c):
        score += 1
...
def grade_q2(method1, text1, method2, text2):
    ...
    def evaluate_sentence(method, text, sent_num):
        ...
        if method in methods_structure and re.search(methods_structure[method], text):
            s += 1
        ...
        # 오개념 판별 (기각 조건)
        if re.search(synonyms["easy_task"], text) and re.search(synonyms["inhibition"], text):
            f.append(f"❌ 문장 {sent_num} 내용 오류: 쉬운 과제와 사회적 억제를 잘못 연결했습니다.")
            s = 0 
        elif re.search(synonyms["hard_task"], text) and re.search(synonyms["facilitation"], text):
            f.append(f"❌ 문장 {sent_num} 내용 오류: 어려운 과제와 사회적 촉진을 잘못 연결했습니다.")
            s = 0
        else:
            # 본문 내용 기반 확인 (결론 방향 확인)
            if re.search(f"{synonyms['easy_task']}|{synonyms['hard_task']}", text):
                s += 1
